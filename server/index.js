...
// ─── SIGN IN (existing accounts only) ────────────────────────────────────────
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        if (exists) return res.status(409).json({ error: 'An account with this email already exists. Please sign in.' });
...
app.delete('/api/vehicles/:id', auth, async (req, res) => {
    await Vehicle.findByIdAndDelete(req.params.id); res.json({ success: true });
});
...
app.delete('/api/drivers/:id', auth, async (req, res) => {
    await Driver.findByIdAndDelete(req.params.id); res.json({ success: true });
});
    ...
    try {
        ...
        if (status === 'Completed' || status === 'Cancelled') {
            if (trip.vehicleId) {
                ...
                await Vehicle.findByIdAndUpdate(trip.vehicleId, vUp);
            }
            if (trip.driverId) {
...
app.delete('/api/trips/:id', auth, async (req, res) => {
    await Trip.findByIdAndDelete(req.params.id); res.json({ success: true });
});
...
app.delete('/api/maintenance/:id', auth, async (req, res) => {
    await Maintenance.findByIdAndDelete(req.params.id); res.json({ success: true });
});
...
app.delete('/api/fuel/:id', auth, async (req, res) => {
    await Fuel.findByIdAndDelete(req.params.id); res.json({ success: true });
});
